...
function PostCompose() {
  ...
  async function compressImageFileTest(images) {
    ...
    try {
      ...
      fileToBase64(compressedFile, (err, result) => {
        if (result) {
          // setMultiImagesUrl(result)
          // setFile(result);
          //   console.log(file);
          //   console.log(String(result.split(",")[1]));
          // setFile64StringWithType(result);
          // setFile64String(String(result.split(",")[1]));
          // setMultiImagesUrl(String(result.split(",")[1]))
          console.log(String(result.split(",")[1]), "log");
          let url = `${result}`
          // setMultiImagesUrl([...MultiImagesUrl, url])
          marray.push(url)
  ...
  async function multiHandle(i) {
    const compressedImages = [];
    var test0 = compressImageFileTest(i[0])
    var test1 = compressImageFileTest(i[1])
    var test2 = compressImageFileTest(i[2])
    var test3 = compressImageFileTest(i[3])
    console.log(test0, "0");
    if (i && i.length > 0) {
      i.map((item) => {
        compressImageFileTest(i[item])
        //  compressedImages.push(test)
        //   //     // onUploadFileChange(item)
      })
    }
    console.log(compressedImages, "item");
    // setMultiImagesUrl(compressedImages)
  }
  return (
    <div>
